...
<?php 
    ...
    $sql_output= "SELECT * FROM " . $select_line . "_hourly_output " . "WHERE time BETWEEN '$select_month 06:00:00' AND '$first_month_next 06:00:00' ORDER BY time DESC";
    // echo $sql_output;
    ...
    for($i = 0; $i < count($arr_output_day); $i++){
        $perform_daily = round(($arr_output_day[$i] / $data_output_standard) * 100, 2);
    ...
    //-------------------------------Lọc data short_dt------------------------------------
    $sql_short_dt = "SELECT * FROM " . $select_line . "_long_downtime " . "WHERE time BETWEEN '$select_month 06:00:00' AND '$first_month_next 06:00:00' AND period < 300 ORDER BY time DESC";
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_short_dt); $x++){
            if($data_short_dt[$x][2] >= ($arr_day[$i] . " 06:00:00") && $data_short_dt[$x][2] < ($arr_day[$i+1] . " 06:00:00")){
                $period_shortDt_daily += round(($data_short_dt[$x][0] / 60 / 1275) * 100, 2);
    ...
    //-------------------------------Lọc data long_dt------------------------------------
    $sql_long_dt = "SELECT * FROM " . $select_line . "_long_downtime " . "WHERE time BETWEEN '$select_month 06:00:00' AND '$first_month_next 06:00:00' AND period > 300 ORDER BY time DESC";
    // echo $sql_long_dt . "</br>";
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_long_dt); $x++){
            if($data_long_dt[$x][2] >= ($arr_day[$i] . " 06:00:00") && $data_long_dt[$x][2] < ($arr_day[$i+1] . " 06:00:00")){
                $period_longDt_daily += round(($data_long_dt[$x][0] / 60 / 1275) * 100, 2);
                // echo $period_shortDt_daily . "<br>";
            }
        }
    ...
    //-------------------------------Lọc data output_ng------------------------------------
    $sql_output_ng = "SELECT * FROM " . $select_line . "_output_ng_total " . "WHERE time BETWEEN '$select_month 06:00:00' AND '$first_month_next 06:00:00' ORDER BY time DESC";
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_output_ng); $x++){
            if($data_output_ng[$x][1] >= ($arr_day[$i] . " 06:00:00") && $data_output_ng[$x][1] < ($arr_day[$i+1] . " 06:00:00")){
                ...
                $period_outputNg_daily = round(($period_outputNg_daily * 3.8 / 60 / 1275) * 100, 2);
    ...
    //-------------------------------Lọc data speed------------------------------------
    $sql_speed = "SELECT * FROM " . $select_line . "_summary_speed " . "WHERE time BETWEEN '$select_month 06:00:00' 
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_speed); $x++){
            if($data_speed[$x][1] >= ($arr_day[$i] . " 06:00:00") && $data_speed[$x][1] < ($arr_day[$i+1] . " 06:00:00")){
                $period_speed_daily += round(($data_speed[$x][0] * 3.8 / 60 / 1275) * 100, 2);
    ...
    //-------------------------------Lọc data prepare------------------------------------
    $sql_prepare = "SELECT * FROM " . $select_line . "_line_prepare " . "WHERE time BETWEEN '$select_month 06:00:00' 
            AND '$first_month_next 06:00:00' ORDER BY time DESC";
    // echo $sql_prepare . "</br>";
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_prepare); $x++){
            if($data_prepare[$x][1] >= ($arr_day[$i] . " 06:00:00") && $data_prepare[$x][1] < ($arr_day[$i+1] . " 06:00:00")){
                $period_prepare_daily += round(($data_prepare[$x][0] / 60 / 1275) * 100, 2);
                // echo $period_speed_daily . "<br>";
            }
        }
    ...
    //-------------------------------Lọc data changing------------------------------------
    $sql_wire_electrode = "SELECT * FROM " . $select_line . "_long_downtime " . "WHERE id_name IN (1177, 1178, 1228) AND time BETWEEN '$select_month 06:00:00' 
            AND '$first_month_next 06:00:00' ORDER BY time DESC";
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_wire_electrode); $x++){
            if($data_wire_electrode[$x][1] >= ($arr_day[$i] . " 06:00:00") && $data_wire_electrode[$x][1] < ($arr_day[$i+1] . " 06:00:00")){
                $period_wire_electrode_daily += round(($data_wire_electrode[$x][0] / 60 / 1275) * 100, 2);
                // echo $period_speed_daily . "<br>";
            }
        }
    ...
    //data change code
    $sql_change_code = "SELECT * FROM " . $select_line . "_code_change " . "WHERE time BETWEEN '$select_month 06:00:00' 
            AND '$first_month_next 06:00:00' ORDER BY time DESC";
    ...
    for($i = 0; $i < count($arr_day)-1; $i++){
        ...
        for($x = 0; $x < count($data_change_code); $x++){
            if($data_change_code[$x][1] >= ($arr_day[$i] . " 06:00:00") && $data_change_code[$x][1] < ($arr_day[$i+1] . " 06:00:00")){
                $period_change_code_daily += round(($data_change_code[$x][0] / 60 / 1275) * 100, 2);
                // echo $period_speed_daily . "<br>";
            }
        }
