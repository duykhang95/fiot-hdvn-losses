<?php
    ...
    for ($i = 0 ; $i < count($data_long_downtime); $i++){
        if (!in_array($data_long_downtime[$i][2], $arr_tmp1, true)){
            array_push($arr_tmp1, $data_long_downtime[$i][2]);
            array_push($arr_tmp2, array($data_long_downtime[$i][2], 0 ,0));
        }
    }

    for ($i = 0 ; $i < count($data_long_downtime); $i++){
        if (in_array($data_long_downtime[$i][2], $arr_tmp1, true)){
            $temp1 = array_search($data_long_downtime[$i][2], $arr_tmp1);
            // echo $temp1 . "</br>";
            $arr_tmp2[$temp1][1]+=1;
            $arr_tmp2[$temp1][2]+= (int)$data_long_downtime[$i][6];
        }
    }
...
<div class="col-sm-12">
    <div id="chart_long_downtime" style="width: 100%; height: 65vh; margin-top: 10px;">
    </div>       
</div>
...
<script type="text/javascript">
    Highcharts.chart('chart_long_downtime', {
        chart: {
            type: 'column',
            // backgroundColor: {
        ...
        title: {
            text: 'BIỂU ĐỒ TỔN THẤT DỪNG DÀI <br> 大停止ロスグラフ',
            style:{
                fontSize: '20px',
                color: 'black',
                fontWeight: 'bold'
