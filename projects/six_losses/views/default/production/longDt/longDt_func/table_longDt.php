...
?>
    ...
    <div class="col-sm-12">
        <table id="long_dt" class="table table-hover table-bordered text-center">
            <thead>
                <tr>
            ...
            <tbody>
                <?php
                    for($i = 0; $i < count($data_long_downtime); $i++){
                        echo '<tr>';
                        echo '<td>' . $data_long_downtime[$i][1] . '</td>
                            <td>'. $data_long_downtime[$i][2] . '</td>
                            <td>'. $data_long_downtime[$i][3] . '</td>
                            <td>'. $data_long_downtime[$i][4] . '</td>
                            <td>'. $data_long_downtime[$i][5] . '</td>
                            <td>'. round((int)$data_long_downtime[$i][6] / 60, 1) . '</td>
                            <td><button type="button" name="edit" id="edit" class="btn btn-warning btn-xs"
                                onclick ="editlongDT(' . $data_long_downtime[$i][0] . ',' . '\'' . 
                                $data_long_downtime[$i][1] . '\'' . ',' . '\'' . 
                                $data_long_downtime[$i][2] . '\'' . ',' . '\'' . 
                                $data_long_downtime[$i][3] . '\'' . ',' . '\'' . 
                                $data_long_downtime[$i][4] . '\'' . ')"><span class="fa fa-edit"></span></button></td>
                            <td><button type="button" name="delete" id="delete" class="btn btn-danger btn-xs"
                                onclick ="deletelongDT('. $data_long_downtime[$i][0] . ',' .'\'' . 
                                $data_long_downtime[$i][2].'\'' .')"><span class="fa fa-trash"></span></button></td>';
                        echo '</tr>';
                    }
                ?>
            </tbody>
    ...
    <!-- Modal edit data -->
    <form method="post">
        <div class="modal fade" id="edit_long_dt" tabindex="-1" aria-labelledby="exedit_long_dt" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <input type="hidden" id="edit_id" name="edit_id">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exedit_long_dt">Sửa Thông Tin Dừng Dài</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <input id="edit_longDT_function" name="edit_longDT_function" hidden></input>
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
...
<script type="text/javascript">
    createDataTable('long_dt', 10, true);
</script>
<script type="text/javascript">
    function deletelongDT(id_del,content_del) {
        // console.log(id_stt);
    ...
    function editlongDT(edit_id, op_edit, name_edit, name_jp_edit, status_edit) {
