<?php
	...
	$w_06h_30daybefore = date("Y-m-d H:i:s", $today_06h - 62*24*3600);
	...
	$r_aw2 = mysqli_query($connect,"SELECT time FROM " . $select_line . "_summary_speed " . "ORDER BY id ASC LIMIT 1");	
	...
	$q_aw2 = mysqli_query($connect,"SELECT time FROM " . $select_line . "_hourly_speed " . "ORDER BY time DESC LIMIT 1");
	...
	else
	{
		...
		while($h1 <= $today_06h)
	    {
	        ...
	        //$hourly_speed = mysqli_num_rows(mysqli_query($connect,"select * from data_aw2 where time>='$l' AND time<'$l1'"));
	        $q_hourly_speed = mysqli_query($connect,"select sum(period) AS value_sum from " . $select_line . "_summary_speed " . "where op = 99 AND time >='$l' AND time<'$l1'"); 
            while ($row = $q_hourly_speed->fetch_assoc()) {
                $hourly_speed = $row['value_sum'];
                $hourly_speed = round($hourly_speed, 2);
            }
	        $archive = mysqli_query($connect,"INSERT INTO " . $select_line . "_hourly_speed (period, time) VALUES ($hourly_speed, '$l1')");
	        echo '<br>Output from (and include) ', $l,' to ', $l1, ' is ', $hourly_speed, ' pcs.';
	        $h = $h + 3600;
	        $h1 = $h1 + 3600;
	    }
	}
    
    //STEP-5: Delete records older than 2 weeks in aw2_summary_speed
	//Check last_timestamp_hourly in table aw2_hourly_speed
	$hourly_speed5 = mysqli_query($connect,"SELECT time FROM " . $select_line . "_hourly_speed ORDER BY time DESC LIMIT 1");
	while ($row = $hourly_speed5->fetch_assoc()) {
	    $last_hourly_speed5 = $row['time'];
	}
	echo '<br>Last record is ', $last_hourly_speed5;
	
	$last_datetime_output5 = strtotime($last_hourly_speed5);
	echo '<br>Convert last datetime output is ', $last_datetime_output5;
	
	$output5_30daysbefore = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM " . $select_line . "_summary_speed WHERE time < '$w_06h_30daybefore'"));
	...
	if ($last_datetime_output5 > $date_6h_30daybefore)
    {
    	...
    	{
    		$delete_output5_30daysbefore = mysqli_query($connect,"DELETE FROM " . $select_line . "_summary_speed WHERE `time` < '$w_06h_30daybefore'"); 
    		echo '<br>It cleaned ', $output5_30daysbefore, ' old records of `aw2_summary_speed`';
    	} 
    	else 
    	{
