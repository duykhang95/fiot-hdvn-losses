<?php 
    ...
    $interval_change = "SELECT * FROM jatco_interval WHERE op = 2";
    ...
    if ($resultcheck_op && $resultcheck_op->num_rows > 0) {
        ...
        while ($row = $resultcheck_op->fetch_assoc()) {
            ...
            if((float)$data_interval[$i][1] <= (float)$data_interval[$i][3] && (float)$data_interval[$i][3] < (float)$data_interval[$i][2] ) {
                $sql_update_event = "UPDATE `jatco_interval` SET `event_animation` = 1 WHERE `interval_alarm`= '$int_alarm' AND `interval_current` = '$int_current';";
                ...
                $sql_update_event = "UPDATE `jatco_interval` SET `event_animation` = 1 WHERE `interval_stop`= '$int_stop' AND `interval_current` = '$int_current';";
                mysqli_query($connect, $sql_update_event);
                ...
                $sql_update_event = "UPDATE `jatco_interval` SET `event_animation` = 0 WHERE `interval_current` = '$int_current';";
                mysqli_query($connect, $sql_update_event);
                $data_interval[$i][1] = number_format($data_interval[$i][1]);
                $data_interval[$i][2] = number_format($data_interval[$i][2]);
                $data_interval[$i][3] = number_format($data_interval[$i][3]);
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-6" style="margin: 20px 0px;">
                <div class="title mb-30" style="text-align:right;">
                    <a href="index_intervalJATCO">Thay thế định kì JATCO</a>
                </div>
            </div>
        <!-- end col -->
            <div class="col-md-12">
                <div class="card">    
                    <div class="card-body" >
                        ...
                        <table id="data_winding" class="table table-hover">
                            <thead>
                                <tr>
...
<script type="text/javascript">
    $(document).ready(function(){    
        setInterval(function(){
                $("#data_winding").load(" #data_winding > *");
        }, 1000);
    });
</script>
<!-- <script src="
...
<script type="text/javascript">
    createDataTable('data_winding', 10, false);
</script> -->
