<?php 
    ...
    $interval_change = "SELECT * FROM aw2_interval WHERE op = 5";
    ...
    if ($resultcheck_op && $resultcheck_op->num_rows > 0) {
        ...
        while ($row = $resultcheck_op->fetch_assoc()) {
            ...
            if((float)$data_interval[$i][3] < (float)$data_interval[$i][2] ) {
                $sql_update_event = "UPDATE `aw2_interval` SET `event_animation` = 1 WHERE `interval_current` = '$int_current';";
                ...
                $sql_update_event = "UPDATE `aw2_interval ` SET `event_animation` = 1 WHERE `interval_stop`= '$int_stop' AND `interval_current` = '$int_current';";
                mysqli_query($connect, $sql_update_event);
            ...
            // else if((float)$data_interval[$i][3] != (float)$data_interval[$i][1] && (float)$data_interval[$i][3] != (float)$data_interval[$i][2]){
                ...
                $sql_update_event = "UPDATE `aw2_interval` SET `event_animation` = 0 WHERE `interval_current` = '$int_current';";
                mysqli_query($connect, $sql_update_event);
                // $data_interval[$i][1] = number_format($data_interval[$i][1]);
                $data_interval[$i][2] = number_format($data_interval[$i][2]);
                $data_interval[$i][3] = number_format($data_interval[$i][3]);
        ...
        <div class="row align-items-center">
            <div class="col-md-6" style="margin: 20px 0px;">
                <div class="title mb-30">
                    <h3> ĐÚC và KIỂM TRA / 成形工程 & 検査 </h3>
                </div>
            </div>
            <div class="col-md-6" style="margin: 20px 0px;">
                <div class="title mb-30" style="text-align:right;">
                    <a href="index_intervalAW2">Thay thế định kì AISIN 1</a>
                </div>
            </div>
        <!-- end col -->
            <div class="col-md-12">
                <div class="card">    
                    <div class="card-body" >
                        ...
                        <table id="data_molding" class="table table-hover">
                            <thead>
                                <tr>
    ...
    <script type="text/javascript">
        $(document).ready(function(){    
            setInterval(function(){
                  $("#data_molding").load(" #data_molding > *");
            }, 1000);
        });
    </script>
